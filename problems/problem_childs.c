...
#include <unistd.h>
#include <math.h>
#include "/home/childs/rebound/src/rebound.h"
...
int hit_and_run(struct reb_simulation* const r, struct reb_collision c, struct collision_params *params){  //also includes partial accretion.  Mlr = M_target.  Projectile is erroded.
        ...
        if (params->Vi <= v_crit){             //if impact velocity is low, the hit-and-run results in a merger.
            ...
            if (params->Mlr < targ_m){ //erosion of target with fragments            
            ...
            else{ //Mlr >= targ_m, target remains unchanged
                ...
                if (projectile->m-Mlr_dag < min_frag_mass){
...
///POPULATE STRUCT OBJECTS
    ...
    if (Vi <= V_esc){
        ...
        if (b < targ_r){   //non-grazing regime  
            ...
            printf("HEAD-ON SMASHED\n");
            if (targ_m < MAX(Mlr, min_frag_mass)){
                params->collision_type = 2;}
            else{ 
                params->collision_type = 3;}
...
void heartbeat(struct reb_simulation* r){
    if (reb_output_check(r, 1000)){   //heartbeat is at every 1000 years
...
int main(int argc, char* argv[]){
        char* filename = "simulationarchive1.bin";
        ...
        r->G                = 39.476926421373;   //units are now in kg, m, s.
        r->remove_max_distance = 100.; 
        r->integrator         = REB_INTEGRATOR_MERCURIUS;
        r->ri_mercurius.hillfac = 3.; //hybrid integrator switches when close encounter is detected at this hill radii
        r->collision        = REB_COLLISION_DIRECT;
        r->collision_resolve     = reb_collision_resolve_fragment;   // Set function pointer for collision recording.
        r->heartbeat        = heartbeat;
        reb_simulationarchive_automate_interval(r,"simulationarchive1.bin",1000.);
        reb_integrate(r,1e8);
        printf("Done.\n");
}
